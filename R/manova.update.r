...
#' @param PC.no A value that, if not NULL, can override the tolerance argument, and forces a desired number of 
...
manova.update <- function(fit, error = NULL, 
  ...
  sscp <- function(Uf, Ur, Ufull, Unull, Y, n, p, Yt) {
    ss <- Map(function(uf, ur, y) list(SS = crossprod(fastFit(uf, y, n, p.prime) -
                                              fastFit(ur, y, n, p.prime)),
    ...
    result <- c(ss, list(Full.Model = list(SS = tss - rss, Residuals = rss)))
  ...
  getEigs <- function(x) Re(eigen(x, symmetric = FALSE, only.values = TRUE)$values)
  ...
  names(SSCP) <- names(invR.H) <- names(eigs) <- c("obs", paste("iter", 1:(perms - 1), sep = "."))
