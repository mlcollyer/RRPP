...
#' Print/Summary Function for RRPP
#'
#' @param x Object from \code{\link{z.model.test}}
#' @param ... Other arguments passed onto model.comparison
#' @export
#' @author Michael Collyer
#' @keywords utilities
print.z.model.test <- function(x ,...){
  n <- nrow(x$Z)
  cat("\n Pairwise z-tests performed for", n, "models\n\n")
  cat("Models compared:\n")
  print(x$form.names)
}
...
#'
#' @param x Object from \code{\link{z.model.test}}
#' @param stat.table Logical value to indicate whether to summarize stats in one table
#' (default = TRUE) or separate pariwise tables (= FALSE)
#' @param names One of "form" or "model.  The former returns formulae for model names; the
#' latter returns, "m.1", "m.2",...
#' @param ... Other arguments passed onto model.comparison
#' @export
#' @author Michael Collyer
#' @keywords utilities
summary.z.model.test <- function(object , stat.table = TRUE,
                                 names = c("form", "model"), ...){
  x <- object
  n <- nrow(x$Z)
  effect.type <- x$anova$effect.type
  cat("\nPairwise z-tests performed for", n, "models\n")
  cat("Test based on the", effect.type, "statistic sampling-distributions\n")
  cat("RRPP Permutations:", x$anova$perm.number,"\n")
  cat("Models compared:\n\n")
  cat(paste(x$model.names," (",x$form.names,")  ", sep = ""))
  Z <- x$Z
  P <- x$P
  mnames <- match.arg(names)
  if(mnames == "model") nms <- x$model.names else nms <- x$form.names
  rownames(Z) <- colnames(Z) <- rownames(P) <- colnames(P) <- nms
  if(!stat.table){
    cat("\n\nPairwise Z-scores\n")
    print(Z)
    cat("\nPairwise P-values\n")
    ...
    Z <- as.vector(as.dist(Z))
    P <- as.vector(as.dist(P))
    tab <- data.frame(Z=Z, P=P)
...
#' Plot Function for RRPP
#' 
#' @param x plot object (from \code{\link{z.model.test}})
#' @param label One of "form", "model", or "none", to indicate where to lable
#' points by formulae, model sequence, or not at all.
#' @param ... other arguments passed to plot (helpful to employ
#' different colors or symbols for different groups).  See
#' \code{\link{plot.default}} and \code{\link{par}}
#' @export
...
#' @keywords visualization
#' 
plot.z.model.test <- function(x, label = c("none", "model", "form"), ...){
  ...
  dy <- yrange[2] - yrange[1]
  ylim <- yrange + c(-0.1*dy, 0.1*dy)
  plot(xx, yy, asp = 1, xlim = xlim, ylim = ylim,
       xlab = paste("MDS1", round(d[1]*100, 1), "%"),
       ylab = paste("MDS2", round(d[2]*100, 1), "%"),
       ...)
  if(label == "form") lab <- x$form.names else
    if(label == "model") lab <- x$model.names else
      lab <- rep("", nrow(P))
  text(xx, yy, lab, pos = 1, cex = 0.5)
}